...
const STYLES = `
  ...
  *{box-sizing:border-box}
  ...
  .chat-bubble{position:fixed;bottom:24px;right:24px;width:52px;height:52px;border-radius:50%;background:var(--accent);color:white;border:none;cursor:pointer;font-size:22px;display:flex;align-items:center;justify-content:center;box-shadow:0 4px 16px rgba(0,0,0,0.4);z-index:100}
  .chat-panel{position:fixed;bottom:88px;right:24px;width:340px;max-width:calc(100vw - 32px);height:460px;background:var(--surface-2);border:0.5px solid var(--border-strong);border-radius:14px;display:flex;flex-direction:column;box-shadow:0 8px 32px rgba(0,0,0,0.5);z-index:100;overflow:hidden}
  .chat-header{padding:12px 14px;border-bottom:0.5px solid var(--border);display:flex;justify-content:space-between;align-items:center}
  .chat-header span{font-size:13px;font-weight:500}
  .chat-messages{flex:1;overflow-y:auto;padding:12px 14px;display:flex;flex-direction:column;gap:10px}
  .chat-msg{font-size:13px;padding:8px 10px;border-radius:10px;max-width:85%;line-height:1.4;white-space:pre-wrap}
  .chat-msg.user{background:var(--accent-bg);color:var(--text-primary);align-self:flex-end}
  .chat-msg.assistant{background:var(--surface-1);color:var(--text-secondary);align-self:flex-start}
  .chat-input-row{display:flex;gap:6px;padding:10px;border-top:0.5px solid var(--border)}
  .chat-input{flex:1;background:var(--surface-1);border:0.5px solid var(--border);border-radius:8px;color:var(--text-primary);font-family:inherit;font-size:13px;padding:8px 10px;resize:none}
  .chat-input:focus{outline:none;border-color:var(--border-strong)}
  .chat-send{background:var(--accent);color:white;border:none;border-radius:8px;padding:0 14px;cursor:pointer;font-size:13px;font-family:inherit}
...
export default function Dashboard() {
  ...
  const [raisesRefreshing, setRaisesRefreshing] = useState(false);
  const [raisesRefreshError, setRaisesRefreshError] = useState(null);
  ...
  async function refreshRaises() {
    setRaisesRefreshing(true);
    setRaisesRefreshError(null);
    try {
      const res = await fetch('/api/refresh-raises', { method: 'POST' });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || 'Refresh failed');
      const withIds = json.items.map((item, i) => ({ id: Date.now() + i, status: 'Not contacted', ...item }));
      save({ ...data, raises: withIds, lastRaisesRefresh: new Date().toISOString() });
    } catch (err) {
      setRaisesRefreshError(err.message);
    } finally {
      setRaisesRefreshing(false);
    }
  }
  ...
  async function sendChatMessage() {
    ...
    try {
      const res = await fetch('/api/chat', {
        ...
        body: JSON.stringify({ messages: nextMessages, dashboardData: data }),
      });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || 'Chat failed');
      setChatMessages([...nextMessages, { role: 'assistant', content: json.reply }]);
    } catch (err) {
      setChatMessages([...nextMessages, { role: 'assistant', content: `Error: ${err.message}` }]);
    } finally {
  ...
  return (
    <div className="wrap">
      ...
      {activeTab === 'raises' && (
        <>
          <div className="row">
            <p>{data.lastRaisesRefresh ? `Last refreshed ${new Date(data.lastRaisesRefresh).toLocaleString()}` : 'Companies that recently closed rounds, with investors and founder info.'}</p>
            <div style={{ display: 'flex', gap: 8 }}>
              <button className="action" onClick={refreshRaises} disabled={raisesRefreshing}>{raisesRefreshing ? 'Searching...' : '↻ Refresh'}</button>
              <button className="action" onClick={addRaise}>+ Add raise</button>
            </div>
          </div>
          {raisesRefreshError && <div className="error-box">Refresh failed: {raisesRefreshError}</div>}
          {data.raises.length === 0 && !raisesRefreshing && <p className="empty">Nothing here yet. Click refresh or add one manually.</p>}
      ...
      <button className="chat-bubble" onClick={() => setChatOpen(!chatOpen)} aria-label="Toggle chat">
        {chatOpen ? '✕' : '💬'}
      </button>
      ...
      {chatOpen && (
        <div className="chat-panel">
          <div className="chat-header">
            ...
            <button className="iconbtn" onClick={() => setChatOpen(false)}>✕</button>
          </div>
          <div className="chat-messages">
            {chatMessages.length === 0 && (
              <p className="empty">Ask me anything, including questions about your relationships, raises, or news.</p>
            )}
